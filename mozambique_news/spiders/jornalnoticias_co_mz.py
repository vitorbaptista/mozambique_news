...
class JornalnoticiasCoMzSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        try:
            ...
            if news_data['published_at'].replace(tzinfo=None) < start_date.replace(tzinfo=None):
    ...
    def parse_news(self, selector):
        ...
        if not data['body']:
            data['body'] = '\n'.join([row.strip() for row in selector.css('p::text').extract()])
        ...
        data['published_at'] = datetime.datetime.strptime(data['published_at'], '%Y-%m-%dT%H:%M:%S%z')
